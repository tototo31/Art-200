...
void setup()
{
  size(800, 600);
  smooth();
  snowflake = new Snowflake(len, scale);
...
void draw()
{
  ...
  case 1: // simulation state
    ...
    if (frameCount%freq == 1)  // determine when to drop flakes
    {
      ...
      flake.setAllowSleeping(true);
      //flake.attachImage(snowflake.flake);
    ...
    for (int i = 0; i < bodies.size(); i++) // for each body draw a flake over it
    {
      ...
      {
        snowflake.display(bodies.get(i).getX(), bodies.get(i).getY()); // if it wasnt removed show the beautiful flake
